...
protein_concentration = (1e-6)/2.5^2; % protein concentration in plate (M)
protein_volume = 150e-6; % protein volume in plate (L)
...
titrant_concentrations = 2.^(0:20) * 1e-9; % fluorescent ligand concentrations (M)
titrant_volumes        = 10 * ones(1,21) * 1e-6; % fluorescent ligand titrant volumes (L)
...
%titrant_concentrations = [1e-9, 10e-9, 100e-9, 1e-6, 10e-6, 100e-6]; % fluorescent ligand concentrations (M)
...
title(sprintf('%.0f uL of %.3f nM protein', protein_volume / 1e-6, protein_concentration / 1e-9));
...
exportfig(gcf, filename, 'width', 14, 'height', 14, 'color', 'cmyk');
