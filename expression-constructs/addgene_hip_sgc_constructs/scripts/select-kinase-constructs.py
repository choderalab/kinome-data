...
addgene_hip_plasmids = pd.concat((selected_addgene_plasmids, hip_plasmids))
addgene_hip_plasmids.reset_index(inplace=True)

plasmid_source = ['addgene'] * len(selected_addgene_plasmids) + ['HIP pJP1520'] * len(hip_plasmids)
addgene_hip_plasmids['plasmid_source'] = pd.Series(plasmid_source)
...
for target in pdbconstructs_xml:
    ...
    # get plasmid data
    matching_addgene_hip_plasmids = addgene_hip_plasmids[ addgene_hip_plasmids['matching_targetID'] == targetID ]
    matching_addgene_plasmids = matching_addgene_hip_plasmids[ matching_addgene_hip_plasmids['plasmid_source'] == 'addgene' ]
    matching_hip_plasmids = matching_addgene_hip_plasmids[ matching_addgene_hip_plasmids['plasmid_source'] == 'HIP pJP1520' ]
    matching_sgc_plasmids = selected_sgc_plasmids[ selected_sgc_plasmids['matching_targetID'] == targetID ]

    matching_addgene_hip_plasmids.sort(('nconflicts_target_domain_region', 'nextraneous_plasmid_residues'), inplace=True)
   ...
   #  top_addgene_expr_tag = matching_addgene_plasmids['Expression tag'].values[0] if len(matching_addgene_plasmids) > 0 else None
    ...
    if len(matching_addgene_hip_plasmids) == 0:
        continue   # skip if no addgene or hip plasmids

    elif top_pdb_auth_score <= 0 and len(matching_sgc_plasmids) == 0:
        continue   # skip if no authentic PDB constructs and no SGC constructs

    elif top_pdb_auth_score <= 0 and len(matching_sgc_plasmids) > 0:
        selected_construct_source = 'SGC'   # if SGC but no PDB

    elif top_pdb_auth_score > 0 and len(matching_sgc_plasmids) == 0:
        selected_construct_source = 'PDB'   # if PDB but no SGC

    elif top_sgc_plasmid_nextraneous_residues <= top_pdb_nextraneous_residues:
        selected_construct_source = 'SGC'   # if SGC and PDB

    elif top_sgc_plasmid_nextraneous_residues > top_pdb_nextraneous_residues:
        selected_construct_source = 'PDB'   # if SGC and PDB

    else:
        print 'WARNING: Shouldn\'t get here'
        import ipdb; ipdb.set_trace()

    top_plasmid = matching_addgene_hip_plasmids.head(1)
