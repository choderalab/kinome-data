...
# copied/pasted data from "Gen9_Seq_Submission_Short Form-choderalab-91_kinases.xlsx"
constructs_bp_lens = array([795,
942,
891,
903,
843,
...
total_bps = sum(constructs_bp_lens)

costs = array(map(construct_cost, constructs_bp_lens, [total_bps] * len(constructs_bp_lens)))
total_price = sum(costs)

print 'Total number of constructs: %d' % len(constructs_bp_lens)
print 'Total number of bps: %d' % sum(constructs_bp_lens)
