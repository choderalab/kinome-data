...
def process_target(t):
    ...
    for i in range(len(cloneIDs)):
        additional_data[0][cloneIDs[i]] = str(int(plasmids['nconflicts_target_domain_region'][i]))
        additional_data[1][cloneIDs[i]] = str(int(plasmids['nextraneous_plasmid_residues'][i]))
